...
%This section was written by original GUS team, copied and clarified by Cindy, and further clarified by Alex
...
postale 56, CH-1211 Genève 20, Switzerland, and The Institute of Electrical and
...
his document is for LIMITED RELEASE ONLY to University of Idaho CS personnel
working on the project and Dr. Clinton Jeffery.

%This section originally written by the Gus team, with additions by Cindy and Alex.
...
%Alex: I think the figures and use cases need to be moved/reworked/reogranized
...
%Cindy: I agree the use cases need to be reorganized, but I suspect that it would be easier to
%maintain traceability if we at least kept one \paragraph or \subsubsubsection per use-case.
%I agree though text is prettier than numbers, so I will make the changes.  Is there a way to
%get an extra line break between paragraphs?

%Alex: The paragraphs look cleaner, to be sure.  I think having a subsection for each use case
%would be alright.  I'm not sure if there's a way to just put a line break in. I tried using \linebreak
%and \newline, but they apparently work as suggestions rather than orders.
%On another note, someone has been keeping minutes and records of our meetings, right?  We could put those
%into an appendix pretty easily and it'd probably help with further filling out of the SSDD.


\subsubsection{User{\textquoteright}s View Representation and Description}
\paragraph{Group Administrator Use Cases}
\paragraph{Add Officer}
\paragraph{Modify Officer Access}
\paragraph{Remove Officer}
\begin{wrapfigure}[26]{r}{4in}
  \begin{center}
\includegraphics{../http/png/officer_control.png}
  \end{center}
  \caption{Officer Panel Mockup by Leah Wegner}
\end{wrapfigure}
\paragraph{Officer Use Cases}
The purpose of the officer module is to enable officers to manage the group events, membership, and website.
Officers can: add, modify, or remove events; add, modify, or remove group websites, and add or remove members.
Because officers can just be a member of a different group, the officer's available actions would be listed in a pop-out menu
connected to the group's site.  To improve useability, the pop-out menu should look like an extention of the user menu.
For example: the officer navigates to the group website, his user menu becomes longer, he hovers over ``events'', the pop-out
would display ``add event, modify event, and remove event'', the officer would then select an option, such as,
``remove event'' and the remove event page would be displayed.
...
likely to attend but didn't sign up, but everyone who RSVPed to include John (who's just taking the class and is not in club),
followed up by an email, and finally the president
...
An officer may want to change the contents of a group's web page.  For example: this year Idaho's Business
...
\paragraph{Add Activity}
\paragraph{Add User}
\paragraph{Add Website}
\pagebreak
\paragraph{Enter Fee Information}
...
\paragraph{Modify Activity}
\paragraph{Modify Website}
...
\paragraph{Use case: notify members concerning dues/fees} from Mike's use cases.
...
\paragraph{Remove Activity}
\paragraph{Remove User}
\paragraph{Remove Website}
\paragraph{Text and E-mail Module}
...
\paragraph{Send Group Email}

\paragraph{Send Text}
\paragraph{User Use Cases}
\paragraph{Create Email}
\paragraph{Edit Email}
...
\end{wrapfigure}
\paragraph{Edit Personal Page}
The user-view class diagram which this entry is based on was designed by Brett Hitchcock,
this entry was written by Cynthia Rempel.\\

The purpose of a profile is to enable the user or group to present a picture of who she or they are,
...
The personal profile is also a way for users to keep their information up-to-date.  For example: if the head coach needs
...
The user-view class diagram which this entry is based on was designed by Brett Hitchcock,
this entry was written by Cynthia Rempel. \\
The purpose of the event group module is to enable users to join groups and events.
...
an application to go to seminary. A marketing major may want to sign up to judge at a DECA event that Business Professionals of
America is hosting to encourage high-schoolers to learn business skills.\\
When a user makes a request to join a group or event, their request is checked against the database as to whether they meet the
criteria to perform the add group or event action.  If the user is qualified, then the change is made in the database, and a confirmation is
displayed on the screen and sent to the user's email, otherwise, the user is notified they do not meet the criteria.\\
Some groups and events are open to any GUS user, but other groups and events may not be open to the public, so there are three levels of
approval to be built in: open, auto-approved if the criteria is met, and approved by officers only.\\

\paragraph{Join Activity}
\paragraph{Join Group}
\paragraph{Leave Group}
\paragraph{Send Email}
\paragraph{Sign Up}
%Cindy: Abhay wanted something on his JavaScript regular expressions -- could we make sure this happens in this chapter or the next?
\pagebreak
\begin{wrapfigure}[30]{r}{4in}
	\begin{center}
\includegraphics{../http/png/registration-form.png}
	\end{center}
	\caption{registration form validated with regular expressions by Abhay Patil}
\end{wrapfigure}
\paragraph{} Use case: register for gus\\
...
Once the user submits the registration form, a javascript is executed, which then checks the user's input against regular expressions.
Finally, once the form is validated it is checked again by GUS for valid information, then finally the information is sent to the database,
then a series of confirmations are sent, and the user is then given a confirmation message that displays the information added to the database.

\paragraph{Forums Use Cases}

%Cindy: John, what type of diagram was your forums module?
%Cindy: Need more text on forums
\paragraph{Forums Module}
\begin{wrapfigure}[21]{l}{2in}
  \begin{center}
    \includegraphics{../pdf_png/forums_use_cases.png}
  \end{center}
  \caption{replace with screen-shots of forums mock-ups}
\end{wrapfigure}
--- diagram which this entry is based on was designed by John Sandmeyer, this entry was written by Cynthia Rempel.\\
The forums module is designed to enable groups to collaborate on group decisions and consolidate member input.
...
To post on the forums page a user would have to be granted permissions by the groups administrator,
...
For the administrator to modify privaledges, she should be on the forums page of the group, ideally,
...
\paragraph{Add Topic}
\paragraph{Edit Post}
\paragraph{Edit Topic}
\paragraph{Modify Privaledges}
\paragraph{Remove Post}
\paragraph{Remove Topic}
...
\begin{figure}[h]
    \includegraphics{../personal_images/Cynthia/SampleClass.png}
	\caption{Sample Boundary Class Diagram by Cynthia}
\end{figure}
\paragraph{Add Officer}
\begin{figure}[h]
    \includegraphics{../personal_images/Zeke/add_officer_sequence}
	\caption{Add Officer Sequence Diagram by Zeke, image linked by Cindy, textual description by ...}
\end{figure}
\paragraph{Modify Officer Access}
\paragraph{Remove Officer}
\paragraph{Change Group password -- implement access through roles} The level of access a person has to
the group's website is implemented through the use of roles: when a user navigates to a group's website, the
system checks with the database to see if the user is an officer, and if so, displays the additonal officer
menu underneath or next to their user menu.
\paragraph{Add User}
\paragraph{Add Activity}\begin{figure}[h]
    \includegraphics{../personal_images/Zeke/add_event_sequence.png}
	\caption{Add Activity Sequence Diagram by Zeke, image linked by Cindy, textual description by ...}
\end{figure}
\paragraph{Add Website}
\paragraph{Modify Real Life Event}
\paragraph{Modify Website}
\paragraph{Remove User}
\paragraph{Remove Website}
\paragraph{Remove Real Life Event}
\paragraph{Send Group Email}
\paragraph{Send Text}
\paragraph{Create Email}
\paragraph{Edit Email}
\paragraph{Edit Personal Page}
\paragraph{Join Real Life Event}
\paragraph{Join Group}
\paragraph{Leave Group}
\paragraph{Send Email}
\paragraph{Sign Up}
...
\paragraph{Modify Privaledges}
\begin{figure}[ht]
    \includegraphics{../doc/proto_framework_associations2.png}
...
\begin{figure}[ht]
    \includegraphics{../doc/form-data-submission-statechart2.png}
