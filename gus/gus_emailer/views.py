...
def email_user(request,user_id):
    ...
    #return HttpResponse(gus_user.objects.get(pk=user_id).email)
    if request.method == 'POST': # If the form has been submitted...
        form = ContactFormUser(request.POST) # A form bound to the POST data
        if form.is_valid(): # All validation rules pass
        ...
        form = ContactFormUser({'recipients':usr.email}) # An unbound form
    
    return render_to_response('email/index.html', {
        'email_form': form, 'user_to':usr,
    }, context_instance=RequestContext(request))
